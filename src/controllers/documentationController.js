const { Filedrive, UserChangeRequest, Documentation, Dispositive, Workplace, Jobs } = require('../models/indexModels');
...
const JOB_DOCUMENTATION_TEMPLATES = [
  {
    key: "prl-info",
...
const buildJobDocumentationName = (template, positionName) => {
  return `${template.prefix} - ${String(positionName || "").trim()}`;
};

const ensureDocumentationForPosition = async (positionId, positionName, template) => {
  if (!positionId || !positionName || !template?.key) return null;

  const cleanName = String(positionName).trim();
  if (!cleanName) return null;

  const existing = await Documentation.findOne({
  ...
  return Documentation.create({
    name: buildJobDocumentationName(template, cleanName),
    model: template.model || "User",
    visible: true,
    date: !!template.date,
